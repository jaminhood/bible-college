...
import { changeAnswer, finishedPaper, unfinishPaper } from '../../redux/questionsSlice'
...
export default function Exam ()
{
  ...
  return (
    <>
      <Page title={`Exam`}>
        <section className='exam-section text-light d-flex justify-content-center align-items-center' style={{ backgroundImage: `url(${bg})` }}>
          <Container>
            <Row>
                    ...
                    ? (
                      <div className="card">
                        <div className="card-header text-center">
                          <p className='display-6 m-0'>GST102 Philosophy</p>
                        </div>
                        {questions.map(question => (
                          <QuestionCard question={question} key={question.id} handleResult={handleResult} handleEssay={handleEssay} />
                        ))}
                        <div className="card-footer">
                          <div className="w-100 d-flex justify-content-end align-items-center">
                            <motion.button whileTap={{ scale: 1.2 }} className="btn btn-success btn-md w-100" onClick={handleFinished}>
                              Finish
                      ...
                      <div className="card">
                        <div className="card-body text-center">
                          <h3 className="display-6 border-bottom pb-2">Examination Completed Successfully</h3>
                          <p className="lead mt-2">Objective: {score} / {questions.length - 1}</p>
                        </div>
                      </div>
                    )}
                </div>
        ...
        <div className="timer bg-danger p-1 text-light text-center">
          <p className="lead m-0">{minutes < 10 ? '0' + minutes : minutes} : {seconds < 10 ? '0' + seconds : seconds} Left</p>
        </div>
      </Page>
    </>
  )
